...
export async function setStartSettings(currentLanguage, currentCurrency) {
  ...
  //const platformLanguage = NativeModules.I18nManager.localeIdentifier;
  
  const deviceLanguage = platformLanguage.split('_')[0];
  reactotron.log({deviceLanguage})
  try {
    ...
    if (!currentCurrency?.currencyCode && currentCurrency?.currencyCode!='') {
    ...
    if (!currentLanguage?.langCode && currentLanguage?.langCode!='') {
      ...
      languages && languages.forEach((el) => {
