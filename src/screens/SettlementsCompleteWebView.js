...
export class SettlementsCompleteWebView extends Component {
  ...
  onNavigationStateChange = ({ nativeEvent }, cart) => {
    ...
    const { return_url, cartActions, orderId, cancel_url, decline_url } = this.props;
    ...
    if (
      nativeEvent.url &&
      cancel_url &&
      nativeEvent.url.toLowerCase().startsWith(decline_url.toLowerCase())
    ) {
  ...
  render() {
    const { url, query_parameters, cart } = this.props;
    //let url = payment_url;
